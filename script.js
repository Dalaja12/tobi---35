...
// === SOPORTE TOUCH PARA COMIDA (MÓVIL) ===
...
function getHumanIntro() {
    ...
    const phrases = [
        `Tal vez esto te sirva${name ? ", " + name : ""} 👇`,
        `Mira lo que encontré${name ? ", " + name : ""} 👀`,
        `Esto es lo que encontré para ti${name ? ", " + name : ""} 👇`,
        `Creo que esto te puede ayudar${name ? ", " + name : ""} 😊`
    ];
...
function playAnswer() {
    ...
    // 🔥 FORZAR interacción de usuario (PC)
...
// ================================
// 👤 SISTEMA DE USUARIO CYBERPET
// ================================
document.addEventListener("DOMContentLoaded", () => {
    setTimeout(checkUserName, 10); // pequeño delay para que todo cargue
});
